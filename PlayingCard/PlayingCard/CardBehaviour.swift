...
//  PlayingCard
//
//  Created by MonsterHulk on 2018-02-03.
...
class CardBehaviour: UIDynamicBehavior {
    lazy var collisionBehaviour: UICollisionBehavior = {
        let behaviour = UICollisionBehavior()   //no argument to initializer
...
//            push.dynamicAnimator? .removeBehavior(push)
            ...
            //dynamicAnimator causememory cycle(being kep in memory by push by closure), the push's action is pointing to the closure,so they are keeping each other in memory  -> [unowned push]
        }
